...
-- TODO: rewrite this if needed, there are some small issues here and there

OmniCC = {
	min = 3, --minimum duration to show text
	font = STANDARD_TEXT_FONT, --cooldown text font
	size = 12, --cooldown text size
}

local function GetFormattedTime(time)
	if (time > 86400) then
		return ( math.floor((time / 86400 + 1)) .. 'd' );
	elseif (time >= 3600) then
		return ( math.floor((time / 3600 + 1)) .. 'h' );
	elseif (time >= 60) then
		return ( math.floor((time / 60 + 1)) ..'m' );
	elseif (time > 5) then
...
local function CreateCooldownCount(cooldown, start, duration)
	...
	if(cooldown.textFrame.icon and cooldown.textFrame.icon:IsShown()) then
